...
class VehicleProcessing:
    ...
    def update(self, frame):
        ...
        det_results = self.predictor(frame)[0]
        ...
        self.classify_color(frame, boxes)
    ...
    def classify_color(self, frame, boxes):
        ...
        for box in boxes:
            ...
            # get more information about ground
            x1 = max(x1 - 10, 0)
            y1 = max(y1 - 10, 0)
            x2 = min(x2 + 10, w)
            y2 = min(y2 + 10, h)
            ...
            # classify color only for new detection object
            if track_id not in self.color_dict:
                color = self.color_classifier.classify(crop)
    ...
    def draw(self, frame, boxes):
        ...
        for box in boxes:
            ...
            cv2.putText(frame, f"{int(track_id)}-{names[cls]}-{color}", (x1, y1 - 3), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
if __name__ == "__main__":
    ...
    predictor = DetectionPredictor(overrides=cfg)
    # warmup
    ...
    cap = cv2.VideoCapture(videopath)
    print(f"{'preprocess':>15}{'inference':>15}{'postprocess':>15}{'association':>15}")
